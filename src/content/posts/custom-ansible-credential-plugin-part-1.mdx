...
image: /images/posts/custom-ansible-credential-plugin/ansible_credential_plugin_diagram.png
categories:
  - DevOps
  - Ansible
  - Vault
...
<Notice type="info">
  This series was based on a plugin developed for Ansible Tower. However, it is
  still applicable to both AWX and AAP. I'll come back later and make updates
  for AAP specifically.
</Notice>
...
<Notice type="tip">
  Custom Credential plugins in AAP are great when you want to perform a set of
  actions to generate or retreieve the credential that's passed to a playbook.
  As with all plugins within AAP, they are written in Python which means that
  what you can accomplish within a plugin is quite extensive and flexible.
</Notice>
...
This can be found [here](https://github.com/ansible/awx-custom-credential-plugin-example.git).
...
![example AAP credential plugin creation page](/images/posts/custom-ansible-credential-plugin/example_awx_credential.png)
...
![example AAP credential](/images/posts/custom-ansible-credential-plugin/example_machine_credential_username_cred.png)
...
![example AAP credential using custom AAP credential plugin metadata](/images/posts/custom-ansible-credential-plugin/example_machine_credential_username_metadata.png)
...
![finished AAP credential configuration page](/images/posts/custom-ansible-credential-plugin/example_machine_credential_finished.png)
...
![AAP (aka AWX or Tower) custom credential type](/images/posts/custom-ansible-credential-plugin/example_credential_type.png)
...
![custom aap credential type example](/images/posts/custom-ansible-credential-plugin/custom_type_example_credential.png)

##### The credential is added to the job template:

![custom aap credential type example job template](/images/posts/custom-ansible-credential-plugin/example_credential_type_job_template.png)

##### Here's the [playbook](https://github.com/chrisdola/ansible-playbooks/blob/main/example-credential-plugin-use.yml):
...
![custom aap credential type examplec job template](/images/posts/custom-ansible-credential-plugin/ansible_example_credential_job_output.png)
...
- [AAP Creddential Type docs](https://docs.ansible.com/automation-controller/4.0.0/html/userguide/credentials.html#credential-types)
- [My example playbooks repo](https://github.com/chrisdola/ansible-playbooks/)
- [AWX credential plugin example repo](https://github.com/ansible/awx-custom-credential-plugin-example/tree/master)
